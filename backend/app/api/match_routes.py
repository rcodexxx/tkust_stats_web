...
def record_match():
    ...
    if not data:
        ...
        return (
            jsonify(
                {
                    "error": "Request payload must be JSON and Content-Type must be application/json"
                }
            ),
            400,
        )
    ...
    side_a_games_won_val = data.get(
        "side_a_games_won"
    )  # 前端 payload 使用的是 games_won
    ...
    if not match_type_enum:
        errors["match_type"] = (
            f"Invalid match_type: '{match_type_str}'. Valid are: {[e.name for e in MatchTypeEnum]}."
        )

    match_format_enum = MatchFormatEnum.get_by_name(match_format_str)
    if not match_format_enum:
        errors["match_format"] = (
            f"Invalid match_format: '{match_format_str}'. Valid are: {[e.name for e in MatchFormatEnum]}."
        )

    side_a_outcome_enum = OutcomeEnum.get_by_name(side_a_outcome_str)
    if not side_a_outcome_enum or side_a_outcome_enum not in [
        OutcomeEnum.WIN,
        OutcomeEnum.LOSS,
    ]:
        errors["side_a_outcome"] = (
            f"Outcome must be WIN or LOSS: '{side_a_outcome_str}'."
        )
    ...
    try:
        ...
        if side_a_p2_id_str is not None:
            side_a_p2_id = int(side_a_p2_id_str)
        if side_b_p2_id_str is not None:
            side_b_p2_id = int(side_b_p2_id_str)
        ...
        if errors:  # 再次檢查，因為上面可能已經有其他錯誤
            current_app.logger.warning(
                f"Player ID conversion failed or other errors: {errors}"
            )
            return (
                jsonify({"message": "Input validation failed", "errors": errors}),
                400,
            )
    ...
    if side_a_p1:
        player_ids_in_match.add(side_a_p1.id)
    if side_b_p1:
        player_ids_in_match.add(side_b_p1.id)

    if match_type_enum == MatchTypeEnum.DOUBLES:
        if not side_a_p2_id:
            errors.setdefault("side_a_player2_id", []).append(
                "Side A Player 2 is required for doubles."
            )
        ...
        if not side_b_p2_id:
            errors.setdefault("side_b_player2_id", []).append(
                "Side B Player 2 is required for doubles."
            )
        ...
        if side_a_p2_id is not None:
            errors.setdefault("side_a_player2_id", []).append(
                "Singles match should not have Side A Player 2."
            )
        if side_b_p2_id is not None:
            errors.setdefault("side_b_player2_id", []).append(
    ...
    if side_a_p1 and side_b_p1:  # 確保主要球員存在才進行比較
        ...
        if side_a_ids_set.intersection(side_b_ids_set):
            errors.setdefault("players", []).append(
                "A player cannot be on both competing sides."
            )
    ...
    # 計算實際參與的有效球員數量
    actual_valid_players = sum(
        1 for p in [side_a_p1, side_a_p2, side_b_p1, side_b_p2] if p
    )
    ...
    if match_type_enum and len(player_ids_in_match) != expected_player_count:
        ...
        if (
            not errors.get("players") and actual_valid_players == expected_player_count
        ):  # 避免因找不到球員而重複報錯
            errors.setdefault("players", []).append(
    ...
    try:
        if side_a_games_won_val is not None:
            side_a_games_won = int(side_a_games_won_val)
        if side_b_games_won_val is not None:
            side_b_games_won = int(side_b_games_won_val)
    ...
    if errors:  # 最終檢查所有累積的錯誤
        current_app.logger.warning(
            f"Validation failed for /api/matches/record. Errors: {errors}"
        )
        return jsonify({"message": "Input validation failed", "errors": errors}), 400
    ...
    try:
        ...
        # 更新 TrueSkill 評分
        if match_type_enum == MatchTypeEnum.SINGLES:
            ((new_a1_mu, new_a1_sigma), (new_b1_mu, new_b1_sigma)) = (
            ...
            if not (side_a_p1 and side_a_p2 and side_b_p1 and side_b_p2):
                ...
                current_app.logger.error(
                    "Attempted to calculate doubles rating with missing player objects."
                )
                return (
                    jsonify(
                        {
                            "message": "Internal error: Missing player data for doubles rating."
                        }
                    ),
                    500,
                )
        ...
        db.session.rollback()
        current_app.logger.error(
            f"IntegrityError recording match: {str(e.orig)}", exc_info=True
        )
        return (
            jsonify(
                {"message": "Database integrity error.", "error_code": "DB_INTEGRITY"}
            ),
            409,
        )
        ...
        db.session.rollback()
        current_app.logger.error(
            f"Unexpected error recording match: {str(e)}", exc_info=True
        )
        return (
            jsonify(
                {
