        ...
        if (status === 401 && originalRequest.url !== '/auth/refresh' && !originalRequest._retryRefresh) {
            ...
            try {
                ...
                if (newAccessToken) {
                    ...
                    console.warn("Axios Interceptor: Refresh token failed or not available. Logout should have been triggered.");
                    return Promise.reject(error); // 繼續拋出原始錯誤，讓調用方知道
                }
            } catch (storeError) {
                console.error("Axios Interceptor: Error during store interaction or refresh attempt.", storeError);
